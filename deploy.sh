...
generate_config() {
    ...
    local ssl_proxy_line=""
    if [ "$PROTOCOL" = "https" ]; then
        ssl_proxy_line="\$CFG->sslproxy = true;"
    fi
...
generate_env_file() {
    ...
    local extra_80=80
    local extra_443=443
    if [ "$PORT" = "80" ]; then extra_80=8080; fi
    if [ "$PORT" = "443" ]; then extra_443=8443; fi
...
generate_caddyfile() {
    local version_dir="$VERSIONS_DIR/$SELECTED_VERSION"
    local caddy_file="$version_dir/Caddyfile"
    
    if [ "$PROTOCOL" = "https" ]; then
        log_info "Generating Caddyfile for HTTPS..."
        local caddy_domain="${DOMAIN}"
        if [ "$PORT" != "443" ] && [ "$PORT" != "80" ]; then
            caddy_domain="${DOMAIN}:${PORT}"
        fi
...
EOF
        ...
        log_info "Generating Caddyfile for HTTP..."
        cat > "$caddy_file" << EOF
:${PORT} {
    reverse_proxy web:80
}
...
deploy() {
    ...
    echo ""
    echo -e "  ${YELLOW}Fix file permissions for Moodle to work properly:${NC}"
    echo "    sudo chown -R www-data:www-data $version_dir/moodle"
    echo ""
